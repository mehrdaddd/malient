...
export var editRemoteImageMiddleware = function (fetcher) { return function (store) { return function (next) { return function (action) {
    if (action.type === EDIT_REMOTE_IMAGE) {
        editRemoteImage(fetcher, store, action);
    }
    return next(action);
}; }; }; };
export function editRemoteImage(fetcher, store, action) {
    var item = action.item, collectionName = action.collectionName;
    var _a = store.getState(), apiUrl = _a.apiUrl, userAuthProvider = _a.userAuthProvider;
    store.dispatch(editorShowLoading(item));
    return userAuthProvider()
        .then(function (auth) { return fetcher.getImage(apiUrl, auth, item.id, collectionName); })
        .then(fileToBase64)
