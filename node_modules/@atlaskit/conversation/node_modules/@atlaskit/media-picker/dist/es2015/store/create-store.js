...
export default function (eventEmitter, context, useNewUploadService) {
    ...
    return createStore(reducers, tslib_1.__assign({}, defaultState, { apiUrl: serviceHost, redirectUrl: redirectUrl, tenantAuthProvider: authProvider, userAuthProvider: userAuthProvider,
        context: context,
        useNewUploadService: useNewUploadService }), composeWithDevTools(applyMiddleware(startAppMiddleware(eventEmitter), getFilesInRecents(fetcher), changeService, changeAccount, changeCloudAccountFolderMiddleware(fetcher), fetchNextCloudFilesPageMiddleware(fetcher), startCloudAccountOAuthFlow(fetcher, cloudService), unlinkCloudAccount(fetcher), getConnectedRemoteAccounts(fetcher), cancelUpload, importFilesMiddleware(eventEmitter, wsProvider), editRemoteImageMiddleware(fetcher), getPreviewMiddleware(fetcher), finalizeUploadMiddleware(fetcher), proxyUploadEvents, handleCloudFetchingEvent, searchGiphy(fetcher), hidePopupMiddleware(eventEmitter), sendUploadEventMiddleware(eventEmitter))));
};
