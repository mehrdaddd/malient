...
exports.arrow = function (direction, endOfTextblock) { return function (state, dispatch) {
    ...
    if (selection instanceof selection_1.GapCursorSelection &&
        ...
        (side === 0 /* RIGHT */
            ? nodeAfter && !utils_1.isIgnored(nodeAfter)
            : nodeBefore && !utils_1.isIgnored(nodeBefore)) &&
        // gap cursor changes block node
        ((side === 0 /* RIGHT */ && selection.side === 0 /* RIGHT */) ||
            (side === 1 /* LEFT */ && selection.side === 1 /* LEFT */))) {
        // reverse cursor position
        nextSelection = new selection_1.GapCursorSelection($pos, side === 0 /* RIGHT */ ? 1 /* LEFT */ : 0 /* RIGHT */);
    }
    else {
        nextSelection = selection_1.GapCursorSelection.findFrom($pos, direction === 0 /* BACKWARD */ ? -1 : 1, mustMove);
        if (!nextSelection) {
            return false;
        }
        var _a = nextSelection.$from, nodeBefore_1 = _a.nodeBefore, nodeAfter_1 = _a.nodeAfter;
        if (side === 0 /* RIGHT */
            ? !nodeBefore_1 || utils_1.isIgnored(nodeBefore_1)
            : !nodeAfter_1 || utils_1.isIgnored(nodeAfter_1)) {
            // reverse cursor position
            nextSelection = new selection_1.GapCursorSelection(nextSelection.$from, side === 0 /* RIGHT */ ? 1 /* LEFT */ : 0 /* RIGHT */);
        }
    }
    dispatch(tr.setSelection(nextSelection).scrollIntoView());
    return true;
}; };
exports.deleteNode = function (direction) { return function (state, dispatch) {
...
exports.setGapCursorForTopLevelBlocks = function (event, editorRef, posAtCoords) { return function (state, dispatch) {
    // plugin is disabled
    ...
    var isAllowed = gapCursorCoords.side === 1 /* LEFT */
        ? $pos.nodeAfter && !utils_1.isIgnored($pos.nodeAfter)
        : $pos.nodeBefore && !utils_1.isIgnored($pos.nodeBefore);
    if (isAllowed && selection_1.GapCursorSelection.valid($pos)) {
        ...
        dispatch(state.tr.setSelection(new selection_1.GapCursorSelection($pos, gapCursorCoords.side)));
        return true;
    }
    return false;
}; };
