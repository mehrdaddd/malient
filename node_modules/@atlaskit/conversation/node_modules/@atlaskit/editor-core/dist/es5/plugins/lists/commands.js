...
var joinPToPreviousListItem = function (state, dispatch) {
    ...
    if ($cut.nodeBefore &&
        ...
        if ($cut.nodeAfter && $cut.nodeAfter.type === paragraph) {
            ...
            var tr = state.tr.step(new prosemirror_transform_1.ReplaceAroundStep($lastNode.pos, $cut.pos + $cut.nodeAfter.nodeSize, $cut.pos + 1, $cut.pos + $cut.nodeAfter.nodeSize - 1, state.tr.doc.slice($lastNode.pos, $cut.pos), 0, true));
...
exports.backspaceKeyCommand = prosemirror_commands_1.chainCommands(
...
commands_1.filter([
    ...
    commands_1.isNthParentOfType('listItem', 1),
], prosemirror_commands_1.chainCommands(deletePreviousEmptyListItem, commands.outdentList())), 
// if we're just inside a paragraph node and backspace, then try to join
// the text to the previous list item, if one exists
commands_1.filter([commands_1.isEmptySelectionAtStart, commands_1.isNthParentOfType('paragraph', 0)], joinPToPreviousListItem));
