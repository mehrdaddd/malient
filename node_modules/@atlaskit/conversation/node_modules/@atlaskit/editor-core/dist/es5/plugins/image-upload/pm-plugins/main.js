...
exports.createMediaNode = function (url, schema) {
    ...
    ImageUploadState.prototype.addImage = function (view) {
        ...
        return function (options) {
            ...
            if (_this.enabled && options.src) {
                var mediaNode = exports.createMediaNode(options.src, state.schema);
                if (mediaNode) {
                    view.dispatch(prosemirror_utils_1.safeInsert(mediaNode, state.selection.$to.pos)(state.tr).scrollIntoView());
                }
            }
        };
    };
...
exports.ImageUploadState = ImageUploadState;
exports.stateKey = new prosemirror_state_1.PluginKey('imageUploadPlugin');
exports.createPlugin = function (schema, options) {
        ...
        props: {
            handleDOMEvents: {
                drop: function (view, event) {
                    ...
                    analytics_1.analyticsService.trackEvent('atlassian.editor.image.drop');
                ...
                paste: function (view, event) {
                    var pluginState = exports.stateKey.getState(view.state);
                    if (!clipboard_1.isPastedFile(event) || !pluginState.enabled) {
                        return false;
                    }
                    analytics_1.analyticsService.trackEvent('atlassian.editor.image.paste');
                    event.preventDefault();
                    event.stopPropagation();
                    pluginState.handleImageUpload(view, event);
                    return true;
