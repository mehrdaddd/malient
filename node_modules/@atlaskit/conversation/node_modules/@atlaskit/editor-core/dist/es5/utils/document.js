...
exports.preprocessDoc = function (schema, origDoc) {
    if (!origDoc) {
        return;
    }
    var content = [];
    ...
    return schema.nodes.doc.create({}, prosemirror_model_1.Fragment.fromArray(content));
};
function processRawValue(schema, value) {
    if (!value) {
        return;
    }
    var node;
    ...
    try {
        var parsedDoc = prosemirror_model_1.Node.fromJSON(schema, node);
