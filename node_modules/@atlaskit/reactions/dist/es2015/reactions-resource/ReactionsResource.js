        ...
        if (!this.inFlightDetailsRequests[reactionId]) {
            this.inFlightDetailsRequests[reactionId] = this.getDetailedReaction(reaction).then(function (reactionDetails) {
                ...
                if (state.status === ReactionStatus.ready) {
                    var reactionIndex = findIndex(state.reactions, function (r) { return r.emojiId === emojiId; });
                    if (reactionIndex !== -1) {
                        var existingReaction = state.reactions[reactionIndex];
                        if (!existingReaction.optimisticallyUpdated) {
                            state.reactions = updateReadonlyArray(state.reactions, reactionIndex, function (_) { return reactionDetails; });
...
}(AbstractReactionsResource));
export default ReactionsResource;
