...
exports.BreakoutProvider = nodes_1.BreakoutProvider;
...
var ReactSerializer = /** @class */ (function () {
    ...
    ReactSerializer.prototype.serializeFragment = function (fragment, props, target, key) {
        ...
        var emojiBlock = nodes_1.isEmojiDoc(fragment, props);
        var content = ReactSerializer.getChildNodes(fragment).map(function (node, index) {
            ...
            return _this.serializeFragment(node.content, props, nodes_1.toReact(node), node.type.name + "-" + index);
        });
    ...
    ReactSerializer.prototype.getEmojiBlockProps = function (node) {
        return tslib_1.__assign({}, this.getProps(node), { fitToHeight: utils_1.bigEmojiHeight });
    };
    ReactSerializer.prototype.getTableProps = function (node) {
        return tslib_1.__assign({}, this.getProps(node), { columnWidths: editor_common_1.calcTableColumnWidths(node) });
    };
        ...
        return content.reduce(function (acc, node, index) {
            ...
            if (nodeMarks.length === 0) {
                ...
                nodeMarks.forEach(function (mark, markIndex) {
                    var isSameAsPrevious = editor_common_1.isSameMark(mark, currentMarkNode);
                    var previousIsInMarks = markIndex > 0 &&
                        nodeMarks.some(function (m) { return editor_common_1.isSameMark(m, currentMarkNode); });
                    if (!isSameAsPrevious) {
