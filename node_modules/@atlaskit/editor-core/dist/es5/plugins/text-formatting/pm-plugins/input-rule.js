...
function addMark(markType, schema, charSize, char) {
    return function (state, match, start, end) {
        var prefix = match[1], textWithCombo = match[2];
        var to = end;
...
function addCodeMark(markType, schema, specialChar) {
    return function (state, match, start, end) {
        ...
        return transform_to_code_1.transformToCodeAction(state, regexStart, end)
            .delete(regexStart, regexStart + specialChar.length)
            .removeStoredMark(markType);
    };
}
exports.strongRegex1 = /(\S*)(\_\_(\S.*\S|\S)\_\_)$/;
exports.strongRegex2 = /(\S*)(\*\*(\S.*\S|\S)\*\*)$/;
exports.italicRegex1 = /(\S*[^\s\_]*)(\_([^\s\_].*[^\s\_]|[^\_])\_)$/;
exports.italicRegex2 = /(\S*[^\s\*]*)(\*([^\s\*].*[^\s\*]|[^\*])\*)$/;
...
function inputRulePlugin(schema) {
    ...
    if (schema.marks.strong) {
        ...
        var markLength = 2;
        rules.push(input_rules_1.createInputRule(exports.strongRegex1, addMark(schema.marks.strong, schema, markLength, '__')));
        rules.push(input_rules_1.createInputRule(exports.strongRegex2, addMark(schema.marks.strong, schema, markLength, '**')));
    }
    if (schema.marks.em) {
        // *string* or _string_ should italic the text
        var markLength = 1;
        rules.push(input_rules_1.createInputRule(exports.italicRegex1, addMark(schema.marks.em, schema, markLength, '_')));
        rules.push(input_rules_1.createInputRule(exports.italicRegex2, addMark(schema.marks.em, schema, markLength, '*')));
    }
    if (schema.marks.strike) {
        // ~~string~~ should strikethrough the text
        var markLength = 2;
        rules.push(input_rules_1.createInputRule(exports.strikeRegex, addMark(schema.marks.strike, schema, markLength, '~~')));
    }
    if (schema.marks.code) {
        // `string` should monospace the text
        rules.push(input_rules_1.createInputRule(exports.codeRegex, addCodeMark(schema.marks.code, schema, '`')));
    }
    if (rules.length !== 0) {
