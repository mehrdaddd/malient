...
function keymapPlugin(schema) {
    ...
    keymaps.bindKeymapWithCommand(keymaps.backspace.common, commands.removeEmptyHeadingAtStartOfDocument, list);
    [
        ...
        if (schema.nodes[blockType.nodeName]) {
            var shortcut = keymaps.findShortcutByDescription(blockType.title);
            if (shortcut) {
                var eventName = analyticsEventName(blockType.name, 'keyboard');
                keymaps.bindKeymapWithCommand(shortcut, analytics_1.trackAndInvoke(eventName, commands.insertBlockType(blockType.name)), list);
            }
        }
    });
