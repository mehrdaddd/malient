...
export function removeLayoutFromAllChildren(node) {
...
export function transformSliceToRemoveOpenLayoutNodes(slice, schema) {
    ...
    if (slice.openStart && slice.openEnd && slice.content.childCount === 1) {
        ...
        if (maybeLayoutSection.type === schema.nodes.layoutSection) {
            return new Slice(flatmap(slice.content, removeLayoutFromAllChildren), 
...
export function removeLayoutsIfSelectionIsInLayout(slice, state) {
    // If pasting into a layout, remove any layouts from the slice
    var isSelectionInLayout = hasParentNodeOfType(state.schema.nodes.layoutColumn)(state.selection);
    if (isSelectionInLayout) {
        var sliceWithNoLayouts = new Slice(flatmap(slice.content, removeLayoutFromAllChildren), 
