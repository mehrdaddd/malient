...
var Comparators_1 = require("./internal/Comparators");
var UsageFrequencyTracker_1 = require("./internal/UsageFrequencyTracker");
...
var tokenizerRegex = XRegExp.cache("\\p{Han}|[\\p{L}|\\p{N}]+[\\p{Mn}|']*\\p{L}*", 'gi');
...
var addAllVariants = function (emoji, fnKey, map) {
    ...
    if (type_helpers_1.isEmojiDescriptionWithVariations(emoji)) {
        // map variations too
...
exports.getEmojiVariation = function (emoji, options) {
    if (type_helpers_1.isEmojiDescriptionWithVariations(emoji) && options) {
...
var EmojiRepository = /** @class */ (function () {
    function EmojiRepository(emojis) {
        this.emojis = emojis;
        this.initMembers();
    }
    /**
    ...
    EmojiRepository.prototype.findInCategory = function (categoryId) {
        if (categoryId === constants_1.frequentCategory) {
    ...
    EmojiRepository.prototype.addUnknownEmoji = function (emoji) {
        this.emojis = this.emojis.concat([emoji]);
    ...
    EmojiRepository.prototype.used = function (emoji) {
        ...
        if (this.dynamicCategoryList.indexOf(constants_1.frequentCategory) === -1) {
            setTimeout(function () {
                _this.dynamicCategoryList.push(constants_1.frequentCategory);
            });
        }
    };
    ...
    EmojiRepository.prototype.withAsciiMatch = function (ascii, emojis) {
        ...
        if (asciiEmoji) {
            ...
            result = [asciiEmoji].concat(result);
        }
        return result;
    };
    EmojiRepository.prototype.applySearchOptions = function (emojis, query, options) {
        ...
        if (options.sort === undefined) {
            options.sort = 1 /* Default */;
        }
        var comparator;
        if (options.sort === 1 /* Default */) {
            comparator = Comparators_1.createSearchEmojiComparator(query, this.usageTracker.getOrder());
        }
        else if (options.sort === 2 /* UsageFrequency */) {
            comparator = Comparators_1.createUsageOnlyEmojiComparator(this.usageTracker.getOrder());
        }
        if (comparator) {
        ...
        if (options.skinTone) {
            return emojis.map(function (emoji) {
                return exports.getEmojiVariation(emoji, options);
            });
        }
        return emojis;
    };
    EmojiRepository.prototype.initMembers = function () {
        this.usageTracker = new UsageFrequencyTracker_1.UsageFrequencyTracker();
    ...
    EmojiRepository.prototype.initRepositoryMetadata = function () {
        ...
        if (this.usageTracker.getOrder().length) {
            categorySet.add(constants_1.frequentCategory);
        }
        this.dynamicCategoryList = Array.from(categorySet).filter(function (category) { return constants_1.defaultCategories.indexOf(category) === -1; });
    };
    EmojiRepository.prototype.initSearchIndex = function () {
        this.fullSearch = new js_search_1.Search('id');
        this.fullSearch.tokenizer = Tokenizer;
        this.fullSearch.searchIndex = new js_search_1.UnorderedSearchIndex();
    ...
    EmojiRepository.prototype.addToDynamicCategories = function (emoji) {
        if (constants_1.defaultCategories.indexOf(emoji.category) === -1 &&
...
exports.default = EmojiRepository;
