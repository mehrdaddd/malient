...
var MediaImageLoader_1 = require("./MediaImageLoader");
...
var isUnsupportedBrowser = function (browser) {
    var lowerCaseBrowser = browser.toLowerCase();
...
var BrowserCacheStrategy = /** @class */ (function () {
    function BrowserCacheStrategy(mediaImageLoader) {
        ...
        logger_1.default('BrowserCacheStrategy');
    ...
    BrowserCacheStrategy.browser = new ua_parser_js_1.UAParser()
        .getBrowser()
        .name.toLowerCase();
    return BrowserCacheStrategy;
}());
exports.BrowserCacheStrategy = BrowserCacheStrategy;
...
var MemoryCacheStrategy = /** @class */ (function () {
    function MemoryCacheStrategy(mediaImageLoader) {
        logger_1.default('MemoryCacheStrategy');
        this.mediaImageLoader = mediaImageLoader;
        this.dataURLCache = new lru_fast_1.LRUCache(maxImageCached);
    }
    MemoryCacheStrategy.prototype.loadEmoji = function (emoji, useAlt) {
        ...
        if (dataURL) {
            // Already cached
            return type_helpers_1.convertMediaToImageEmoji(emoji, dataURL, useAlt);
        }
        // Not cached, load
        return this.mediaImageLoader
            .loadMediaImage(mediaPath)
            .then(function (dataURL) {
            var loadedEmoji = type_helpers_1.convertMediaToImageEmoji(emoji, dataURL, useAlt);
            if (dataURL.length <= maxImageSize) {
                // Only cache if not large than max size
                _this.dataURLCache.put(mediaPath, dataURL);
            }
            else {
                logger_1.default('No caching as image is too large', dataURL.length, dataURL.slice(0, 15), emoji.shortName);
            }
            return loadedEmoji;
        })
            .catch(function () {
...
var MediaEmojiCache = /** @class */ (function () {
    function MediaEmojiCache(tokenManager) {
        ...
        logger_1.default('MediaEmojiCache');
        this.mediaImageLoader = new MediaImageLoader_1.default(tokenManager);
    }
    MediaEmojiCache.prototype.loadEmoji = function (emoji, useAlt) {
        ...
        if (type_helpers_1.isPromise(emojiCache)) {
            // Promise based
            return emojiCache
...
exports.default = MediaEmojiCache;
