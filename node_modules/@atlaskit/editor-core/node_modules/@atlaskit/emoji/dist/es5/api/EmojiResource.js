...
var EmojiLoader_1 = require("./EmojiLoader");
var EmojiRepository_1 = require("./EmojiRepository");
var SiteEmojiResource_1 = require("./media/SiteEmojiResource");
/**
 ...
 */
exports.supportsUploadFeature = function (emojiProvider) {
...
var EmojiResource = /** @class */ (function (_super) {
    ...
    function EmojiResource(config) {
        ...
        config.providers.forEach(function (provider, index) {
            var loader = new EmojiLoader_1.default(provider);
    ...
    EmojiResource.prototype.initSiteEmojiResource = function (emojiResponse, provider) {
        if (!this.siteEmojiResource && emojiResponse.mediaApiToken) {
            this.siteEmojiResource = new SiteEmojiResource_1.default(provider, emojiResponse.mediaApiToken);
            ...
            if (emojis.length) {
                ...
                if (type_helpers_1.isPromise(done)) {
                    return done
                        .then(function () {
                        logger_1.default('Primed siteEmojiResource');
                    })
                        .catch(function () {
                        logger_1.default('Failed to prime siteEmojiResource');
                    });
                }
                else {
                    logger_1.default('Already primed siteEmojiResource');
                }
            }
            else {
                logger_1.default('No emoji to prime siteEmojiResource with');
            }
        }
    ...
    EmojiResource.prototype.performRetries = function () {
        ...
        currentRetries.forEach(function (resolveReject, retry) {
            var result = retry();
            if (type_helpers_1.isPromise(result)) {
                result
    ...
    EmojiResource.prototype.loadStoredTone = function () {
        var storedToneString = window.localStorage.getItem(constants_1.selectedToneStorageKey);
        if (storedToneString) {
    ...
    EmojiResource.prototype.loadMediaEmoji = function (emoji, useAlt) {
        if (!this.siteEmojiResource || !type_helpers_1.isMediaEmoji(emoji)) {
            return emoji;
        }
    ...
    EmojiResource.prototype.optimisticMediaRendering = function (emoji, useAlt) {
        ...
        if (type_helpers_1.isPromise(optimistic)) {
    ...
    EmojiResource.prototype.recordSelection = function (emoji) {
        ...
        if (recordConfig) {
            var queryParams = {
                emojiId: type_helpers_1.toEmojiId(emoji),
            };
            var requestInit = {
                method: 'POST',
            };
            return util_service_support_1.utils.requestService(recordConfig, {
    ...
    EmojiResource.prototype.setSelectedTone = function (tone) {
        ...
        if (window.localStorage) {
            try {
                window.localStorage.setItem(constants_1.selectedToneStorageKey, tone ? tone.toString() : '');
            }
            catch (e) {
                // tslint:disable-next-line:no-console
