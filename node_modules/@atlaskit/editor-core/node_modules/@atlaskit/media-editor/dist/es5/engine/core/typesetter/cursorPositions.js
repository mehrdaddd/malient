...
exports.getCursorPositions = function (text, direction, textHelperDiv) {
    if (text.length === 0) {
        return [0];
    }
    ...
    var cursors = detectCursorPositions(util_1.getUtf32CodeUnits(text), direction, rootSpan);
...
var detectCursorPositions = function (text, direction, rootSpan) {
    ...
    var totalLength = exports.calculateTotalLength(groups);
...
var getGroups = function (text, baseDirection, rootSpan) {
    ...
    var startIndices = exports.getGroupStartIndices(baseGroups);
    var groupXMinMax = exports.getGroupXMinMax(baseGroups, baseDirection, rootSpan);
...
var extractBaseGroups = function (text, direction, rootSpan) {
    ...
    var spans = exports.createSpansForCharacters(text, parentSpan);
    var result = [];
    // Data for the group being extracted
    var currentText = [];
    var currentDirection;
    text.forEach(function (character, index) {
        if (exports.isWhiteSpace(character)) {
            ...
            if (currentText.length === 0) {
                ...
                var dir = exports.getDirection(index, spans);
...
exports.getGroupStartIndices = function (baseGroups) {
    var index = 0;
...
exports.getGroupXMinMax = function (baseGroups, direction, rootSpan) {
    // We create a temporary span where we will put spans for every group to get their xmin, xmax
    var parentSpan = document.createElement('span');
    parentSpan.style.direction = direction;
    rootSpan.appendChild(parentSpan);
    // Wrap each group into a span
    var spans = exports.createSpansForGroups(baseGroups, parentSpan);
...
exports.createSpansForCharacters = function (text, parent) {
...
// Wrap each group into a span
exports.createSpansForGroups = function (baseGroups, parent) {
    return baseGroups.map(function (group) {
        var span = document.createElement('span');
...
exports.calculateTotalLength = function (groups) {
    return groups
...
var setInternalCursorPositions = function (group, rootSpan, cursors) {
    ...
    if (group.direction === 'rtl') {
        exports.putInternalPositions(group.text, span, cursors, group.startIndex, function (spanWidth) { return group.xmax - spanWidth; }, // for 'rtl' groups we calculate from the right side
        function (// for 'rtl' groups we calculate from the right side
            pos) { return Math.max(pos, group.xmin); });
    }
    else {
        exports.putInternalPositions(group.text, span, cursors, group.startIndex, function (spanWidth) { return group.xmin + spanWidth; }, // for 'ltr' groups we calculate from the left side
...
exports.putInternalPositions = function (text, span, cursors, startIndex, posCalculator, posLimiter) {
