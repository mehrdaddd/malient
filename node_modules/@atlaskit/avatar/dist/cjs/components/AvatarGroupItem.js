'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _extends2 = require('babel-runtime/helpers/extends');

var _extends3 = _interopRequireDefault(_extends2);

var _getPrototypeOf = require('babel-runtime/core-js/object/get-prototype-of');

var _getPrototypeOf2 = _interopRequireDefault(_getPrototypeOf);

var _classCallCheck2 = require('babel-runtime/helpers/classCallCheck');

var _classCallCheck3 = _interopRequireDefault(_classCallCheck2);

var _createClass2 = require('babel-runtime/helpers/createClass');

var _createClass3 = _interopRequireDefault(_createClass2);

var _possibleConstructorReturn2 = require('babel-runtime/helpers/possibleConstructorReturn');

var _possibleConstructorReturn3 = _interopRequireDefault(_possibleConstructorReturn2);

var _inherits2 = require('babel-runtime/helpers/inherits');

var _inherits3 = _interopRequireDefault(_inherits2);

var _react = require('react');

var _react2 = _interopRequireDefault(_react);

var _dropdownMenu = require('@atlaskit/dropdown-menu');

var _Avatar = require('./Avatar');

var _Avatar2 = _interopRequireDefault(_Avatar);

var _hoc = require('../hoc');

var _AvatarGroup = require('../styled/AvatarGroup');

var _helpers = require('../helpers');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var AvatarGroupItem = function (_Component) {
  (0, _inherits3.default)(AvatarGroupItem, _Component);

  function AvatarGroupItem() {
    (0, _classCallCheck3.default)(this, AvatarGroupItem);
    return (0, _possibleConstructorReturn3.default)(this, (AvatarGroupItem.__proto__ || (0, _getPrototypeOf2.default)(AvatarGroupItem)).apply(this, arguments));
  }

  (0, _createClass3.default)(AvatarGroupItem, [{
    key: 'render',
    value: function render() {
      var _props = this.props,
          avatar = _props.avatar,
          onAvatarClick = _props.onAvatarClick;

      var borderColor = (0, _AvatarGroup.getBackgroundColor)(this.props);
      var enhancedProps = (0, _helpers.getProps)(this);
      return _react2.default.createElement(
        _dropdownMenu.DropdownItem,
        (0, _extends3.default)({
          isInteractive: true
        }, enhancedProps, {
          elemBefore: _react2.default.createElement(_Avatar2.default, (0, _extends3.default)({}, avatar, {
            borderColor: borderColor,
            enableTooltip: false,
            size: 'small'
          })),
          href: avatar.href,
          onClick: function onClick(event) {
            if (typeof onAvatarClick === 'function') {
              onAvatarClick({ event: event, item: avatar });
            }
          },
          rel: avatar.target ? 'noopener noreferrer' : null,
          target: avatar.target
        }),
        avatar.name
      );
    }
  }]);
  return AvatarGroupItem;
}(_react.Component);

exports.default = (0, _hoc.withPseudoState)(AvatarGroupItem);