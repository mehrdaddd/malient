...
var CardViewBase = /** @class */ (function (_super) {
    ...
    function CardViewBase(props) {
        ...
        _this.renderLink = function () {
            ...
            return (React.createElement(LinkCard, { status: status, details: metadata, resizeMode: resizeMode, onRetry: onRetry, appearance: appearance, dimensions: dimensions, actions: actions, selectable: selectable, selected: selected }));
        };
        _this.renderFile = function () {
            ...
            return (React.createElement(FileCard, { status: status, details: metadata, dataURI: dataURI, progress: progress, onRetry: onRetry, resizeMode: resizeMode, appearance: appearance, dimensions: dimensions, actions: actions, selectable: selectable, selected: selected, disableOverlay: disableOverlay }));
        };
        ...
        _this.componentHasMountedAtTime = 0;
        _this.state = {
            hasBeenShown: false,
        };
        return _this;
    ...
    CardViewBase.prototype.fireShowedAnalyticsEvent = function (_a) {
        var status = _a.status;
        if (!this.state.hasBeenShown &&
            (status === 'error' || status === 'complete')) {
            var loadTime = Date.now() - this.componentHasMountedAtTime;
            this.props
                .createAnalyticsEvent({ action: 'shown', loadTime: loadTime })
                .fire('media');
    ...
    CardViewBase.prototype.render = function () {
        ...
        return (React.createElement(Wrapper, { mediaItemType: mediaItemType, breakpointSize: breakpointSize(this.width), appearance: appearance, dimensions: wrapperDimensions, onClick: onClick, onMouseEnter: onMouseEnter }, card));
    };
    return CardViewBase;
}(React.Component));
export { CardViewBase };
