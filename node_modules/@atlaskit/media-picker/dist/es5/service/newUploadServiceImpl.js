...
var uploadServiceFactory_1 = require("./uploadServiceFactory");
var NewUploadServiceImpl = /** @class */ (function () {
    function NewUploadServiceImpl(context, uploadParams) {
        var _this = this;
        this.context = context;
        // Browse listener
        this.onFilePicked = function () {
            if (_this.browserElement) {
                var filesArray = Array.prototype.slice.call(_this.browserElement.files);
                _this.addFiles(filesArray);
            }
        };
        // Dropzone listener
        this.onFileDropped = function (dragEvent) {
            dragEvent.preventDefault();
            dragEvent.stopPropagation();
            _this.emit('file-dropped', dragEvent);
            var filesArray = Array.prototype.slice.call(dragEvent.dataTransfer.files);
            _this.addFiles(filesArray);
        };
    ...
    NewUploadServiceImpl.prototype.addBrowse = function (element) {
        if (this.browserElement) {
            throw new Error('Browse element was already assigned. call removeBrowse() first.');
        }
        this.browserElement = element;
        this.browserElement.addEventListener('change', this.onFilePicked);
    };
    NewUploadServiceImpl.prototype.addDropzone = function (element) {
        if (this.dropzoneElement) {
            throw new Error('Dropzone element was already assigned. call removeDropzone() first.');
        }
        this.dropzoneElement = element;
        this.dropzoneElement.addEventListener('drop', this.onFileDropped);
    };
    NewUploadServiceImpl.prototype.removeDropzone = function () {
        if (!this.dropzoneElement) {
            return;
        }
    ...
    NewUploadServiceImpl.prototype.removeBrowse = function () {
        if (!this.browserElement) {
            return;
        }
    ...
    NewUploadServiceImpl.prototype.emitPreviews = function (cancellableFileUploads) {
        ...
        cancellableFileUploads.forEach(function (cancellableFileUpload) {
            ...
            if (size < uploadServiceFactory_1.MAX_FILE_SIZE_FOR_PREVIEW && mediaType === 'image') {
                ...
                getPreviewFromVideo_1.getPreviewFromVideo(file).then(function (preview) {
                    _this.emit('file-preview-update', {
                        file: mediaFile,
                        preview: preview,
                    });
...
exports.NewUploadServiceImpl = NewUploadServiceImpl;
